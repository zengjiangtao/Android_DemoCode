...
public class CaptureModule extends CameraModule implements
    ...
    //nxp added
    private static final int AcclemeterSamplingPeriod = 5;  //ms;
    private static final int MaxNumTaps = 3;

    private CameraActivity mActivity;
    private AudioManager mAudioManager;
    private MediaPlayer mMediaPlayer; 
    private Vibrator mVibrator;
	private Timer mMultipleShotTimer;
	private SensorFuser mSensorFuser;

    private boolean mTapDisableNotificationTone = false;
    private int mBackupRingmode;
    private long mTapDetectorStartTime;
    private boolean mTakingPictureNow = false;
	private boolean mMultipleShots = false;
	private int mMultipleShotsCount = 0;
    ...
    public void init(CameraActivity activity, boolean isSecureCamera, boolean isCaptureIntent) {
        ...
        mBackupRingmode = mAudioManager.getRingerMode();
        mVibrator = (Vibrator)mContext.getSystemService(Context.VIBRATOR_SERVICE);
		mMultipleShotTimer = new Timer("MultipleShotsTimer");
		
        if(mCameraFacing == Facing.BACK){
            mTapDisableNotificationTone = false;
        }else{
            mTapDisableNotificationTone = true;
        }

		//View mainView = mAppController.getModuleLayoutRoot().findViewById(R.id.preview_content);	
		//if ( mainView != null){
    ...
    public void onShutterButtonClick() {
        //Log.i(TAG,"[NXP] onShutterButtonClick()  entry");
    ...
    private void takePictureNow() {
        //Log.i(TAG,"[NXP] takePictureNow()  entry");
        if (mCamera == null) {
    ...
    public void onRemainingSecondsChanged(int remainingSeconds) {
		//nxp added
		if (!mTapDisableNotificationTone)
		{
		    //Log.i(TAG,"[NXP] onRemainingSecondsChanged() play sound");
    ...
    private void cancelCountDown() {
        //Log.i(TAG,"[NXP] cancelCountDown()  entry");
    ...
    public void onQuickExpose() {
        mMainThread.execute(new Runnable() {
            @Override
            public void run() {
                //Log.i(TAG,"[NXP] onQuickExpose()  run()  entry   mTapDisableNotificationTone=" + mTapDisableNotificationTone);
                // Starts the short version of the capture animation UI.
				if(!mMultipleShots){
                    mAppController.startFlashAnimation(true);
				}
				//nxp added
				//only back camera play notification sound.
				if (!mTapDisableNotificationTone){
                	mMediaActionSound.play(MediaActionSound.SHUTTER_CLICK);
				}
				if(mMultipleShots) {
					/*show mutiple shots hit*/
					String str= String.valueOf(mMultipleShotsCount+1) + "/5";
					mUI.ShowTips(str);

					if ((mMultipleShotsCount<5) && (mMultipleShotTimer != null)){
					    mMultipleShotTimer.schedule(new TakePhotoTask(), 500);
						return;
					}
					mUI.HideTips();
				}
				mTakingPictureNow = false;
				mMultipleShots = false;
				mMultipleShotsCount = 0;
            }
        });
    }
    ...
    private void reopenCamera() {
        Log.i(TAG,"[NXP] reopenCamera()  entry");
        if (mPaused) {
            return;
        }
        AsyncTask.THREAD_POOL_EXECUTOR.execute(new Runnable() {
            @Override
            public void run() {
                Log.i(TAG,"[NXP] reopenCamera() -->  run()");
                closeCamera();
    ...
    private void TapDelayOneSecTakePicture(boolean MultipleShots) {
        Log.i(TAG, "[NXP] TapDelayOneSecTakePicture() mTakingPictureNow=" + mTakingPictureNow);
        if (mTakingPictureNow){
            return;
        }
		
		mTakingPictureNow = true;
		mMultipleShots = MultipleShots;
		mMultipleShotsCount = 0;

		/* play Vibration 200ms. */
		if(mVibrator != null){
		...
		if (mSettingsManager.getInteger(SettingsManager.SCOPE_GLOBAL, Keys.KEY_COUNTDOWN_DURATION) != 0){
			onShutterButtonClick();
		}else{
			if(mMultipleShotTimer != null){
				mMultipleShotTimer.schedule(new TakePhotoTask(), 500);
			}
		}
	} 

    private void EnableTapDetector(){
        Log.i(TAG,"[NXP] EnableTapDetector()  mBackupRingmode = " + mBackupRingmode);
		String Cmd  = "nxp_tap=on";
		String uri = "android.resource://" + mContext.getPackageName() + "/" + R.raw.silence;
		
		if(mOneCameraManager.findFirstCameraFacing(Facing.BACK).getValue().compareTo(Integer.toString(mSettingsManager.getInteger(mAppController.getModuleScope(), Keys.KEY_CAMERA_ID))) == 0){
			Log.i(TAG,"[NXP] EnableTapDetector()  back camera return");
			return;
		}	
		
		//set ringtone mode.
		if (mBackupRingmode != AudioManager.RINGER_MODE_VIBRATE){
			mAudioManager.setRingerMode(AudioManager.RINGER_MODE_VIBRATE);
		}

		//mSensorFuser = new SensorFuser(AcclemeterSamplingPeriod, mContext, mActivity, MaxNumTaps);
		mSensorFuser = new SensorFuser(AcclemeterSamplingPeriod, mContext, MaxNumTaps);
		
		//change to Tap Trigger profile
		mAudioManager.setParameters(Cmd);
		mTapDetectorStartTime = getSessionTime();
        try {
            if (mMediaPlayer == null) {
                mMediaPlayer = MediaPlayer.create(mContext, Uri.parse(uri));
                mMediaPlayer.setOnPreparedListener(preListener);
                mMediaPlayer.setLooping(true);
                Log.i(TAG, "EnableTapDetector()    create MediaPlayer instance!  [" + mMediaPlayer);
            } else {
                Log.i(TAG, "EnableTapDetector()    restart MediaPlayer [" + mMediaPlayer);
                mMediaPlayer.seekTo(0);
                mMediaPlayer.start();
            }
        } catch (Exception e) {
            Log.i(TAG, "EnableTapDetector()    Exception");
            e.printStackTrace();
        }
	}
	
    private void DisableTapDetector(){
        Log.i(TAG,"[NXP] DisableTapDetector()");

		String Cmd  = "nxp_tap=off";
        if (mMediaPlayer == null)
            return;

        try {
            if (mMediaPlayer.isPlaying()) {
                Log.i(TAG, "DisableTapDetector()    release MediaPlayer [" + mMediaPlayer + "]");
		...
		//restore profile
		if (mBackupRingmode != AudioManager.RINGER_MODE_VIBRATE){
			mAudioManager.setRingerMode(mBackupRingmode);
		}

		mSensorFuser.release();
		mSensorFuser = null;
	    mAudioManager.setParameters(Cmd);
	}
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        //Log.i(TAG,"[NXP]  onKeyDown()    keyCode=" + keyCode + "  DeviceID=" + event.getDeviceId());
        switch (keyCode) {
            ...
            //nxp added TapTrigger event
            case KeyEvent.KEYCODE_BUTTON_2:
            case KeyEvent.KEYCODE_BUTTON_4:
                //Log.i(TAG,"[NXP] onKeyDown()  received  KEYCODE_BUTTON_2/4");
                return true;
        }
        return false;
    }
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        Log.i(TAG,"[NXP]  onKeyUp()    keyCode=" + keyCode + "  DeviceName=" + InputDevice.getDevice(event.getDeviceId()).getName());
		int cameraid = mSettingsManager.getInteger(mAppController.getModuleScope(), Keys.KEY_CAMERA_ID);
        switch (keyCode) {
            case KeyEvent.KEYCODE_VOLUME_UP:
            case KeyEvent.KEYCODE_VOLUME_DOWN:
				onShutterButtonClick();
            ...
            //nxp added TapTrigger event
	        case KeyEvent.KEYCODE_BUTTON_2:
                //Log.i(TAG,"[NXP] onKeyUp()  received KEYCODE_BUTTON_2  CurrentTime=" + getSessionTime() + "  mTapDetectorStartTime=" + mTapDetectorStartTime);
				if (InputDevice.getDevice(event.getDeviceId()).getName().equals("tfa98xx-tapdetect-0")){	
					if ((getSessionTime() - mTapDetectorStartTime) > 1000){ 
	                    //Log.i(TAG,"[NXP] onKeyUp()  received KEYCODE_BUTTON_2, then call API  TapDelayOneSecTakePicture()");
	                    if (mSensorFuser != null){
							if (mSensorFuser.addSpkTap(2)){
								TapDelayOneSecTakePicture(false);
							}
							Log.i(TAG, "[NXP]KEYCODE_BUTTON_2 \n" + mSensorFuser.toString());
						}else{
							TapDelayOneSecTakePicture(false);
						}
			        }
                ...
                return true;
	        case KeyEvent.KEYCODE_BUTTON_4:
                //Log.i(TAG,"[NXP] onKeyUp()  received KEYCODE_BUTTON_4  CurrentTime=" + getSessionTime() + "  mTapDetectorStartTime=" + mTapDetectorStartTime);
				if (InputDevice.getDevice(event.getDeviceId()).getName().equals("tfa98xx-tapdetect-0")){	
					if ((getSessionTime() - mTapDetectorStartTime) > 1000){ 
	                    //Log.i(TAG,"[NXP] onKeyUp()  received KEYCODE_BUTTON_4, then call API  TapDelayOneSecTakePicture()");
	                    if (mSensorFuser != null){
							if (mSensorFuser.addSpkTap(3)){
								TapDelayOneSecTakePicture(true);
							}
							Log.i(TAG, "[NXP]KEYCODE_BUTTON_4 \n" + mSensorFuser.toString());
						}else{
							TapDelayOneSecTakePicture(true);
						}
			        }
        ...
        if (mStickyGcamCamera) {
            ...
            return new ButtonManager.ButtonCallback() {
                ...
                public void onStateChanged(int cameraId) {
					...
					//nxp added
					EnableTapDetector();
					mSoundPlayer.play(R.raw.timer_final_second, 0.0f); 
					if(mCameraFacing == Facing.BACK){
						mTapDisableNotificationTone = false;
					}else{
						mTapDisableNotificationTone = true;
